---
name: "Joel Z. Leibo"
affiliation: "Google DeepMind"
tags: [multi-agent, social-simulation, reinforcement-learning, llm-agents, neuroscience]
homepage: ""
scholar: ""
date_updated: 2026-04-12
---

## Research areas

- Multi-agent reinforcement learning and social dynamics
- Generative agent-based modeling
- AI and social science

## Key papers

- [[generative-agent-based-modeling-actions-grounded]] — senior author; Concordia GABM library
- [[machine-culture]] — co-author; Nature Human Behaviour Perspective on machine culture (2023)
- [[persona-generators-generating-diverse-synthetic-personas]] — co-author; Persona Generators (support coverage, AlphaEvolve)

## Recent work

Senior researcher at Google DeepMind working on multi-agent systems and their applications to social science. Co-author of Concordia and of work on social path to human-like artificial general intelligence (Duéñez-Guzmán et al., Nature Machine Intelligence 2023).

## Collaborators

- Alexander Sasha Vezhnevets (Google DeepMind)
- Edgar A. Duéñez-Guzmán (Google DeepMind)
- Simon Osindero (Google DeepMind / University of Toronto)

## My notes

Joel Leibo is one of the key figures bridging multi-agent RL and social simulation at DeepMind. His group's work on Concordia is important because it brings epistemic rigor to the question of what it means for a simulation to "validate" against human behavior.
