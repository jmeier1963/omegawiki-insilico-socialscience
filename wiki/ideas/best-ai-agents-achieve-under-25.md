...
status: failed
origin: 'Migrated from research claim (original status: challenged, confidence: 0.2); proposed in: core-bench-fostering-credibility-published-research'
origin_gaps: []
tags:
- reproducibility
- ai-agent
...
priority: 3
pilot_result: ''
failure_reason: Claim was challenged in the source wiki. The original 21% finding reflects a 2024 snapshot; the claim as stated (under 25%) is now challenged by 2025 results showing near-saturation
linked_experiments: []
date_proposed: 2026-04-23
date_resolved: 2026-05-05
---

> **Migrated from a research claim** (`wiki/claims/best-ai-agents-achieve-under-25.md`) on 2026-06-20. Original claim status `challenged` (confidence 0.2) mapped to idea status `failed`. The claim's structured `evidence`/`confidence`/`conditions` are preserved verbatim in the body sections and in the provenance block at the end of this file.
## Statement
