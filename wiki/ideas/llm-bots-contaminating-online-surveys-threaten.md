...
status: proposed
origin: 'Migrated from research claim (original status: proposed, confidence: 0.65); proposed in: potential-existential-threat-large-language-models, identifying-bots-through-llm-generated-text, llm-driven-bot-infiltration-protecting-web'
origin_gaps: []
tags:
- silicon-sampling
- survey-bots
...
> **Migrated from a research claim** (`wiki/claims/llm-bots-contaminating-online-surveys-threaten.md`) on 2026-06-20. Original claim status `proposed` (confidence 0.65) mapped to idea status `proposed`. The claim's structured `evidence`/`confidence`/`conditions` are preserved verbatim in the body sections and in the provenance block at the end of this file.
## Statement
