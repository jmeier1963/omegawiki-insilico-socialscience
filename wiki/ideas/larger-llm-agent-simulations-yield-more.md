...
status: proposed
origin: 'Migrated from research claim (original status: weakly_supported, confidence: 0.45); proposed in: modeling-earth-scale-human-like-societies'
origin_gaps: []
tags:
...
> **Migrated from a research claim** (`wiki/claims/larger-llm-agent-simulations-yield-more.md`) on 2026-06-20. Original claim status `weakly_supported` (confidence 0.45) mapped to idea status `proposed`. The claim's structured `evidence`/`confidence`/`conditions` are preserved verbatim in the body sections and in the provenance block at the end of this file.
## Statement
